...
// Initialize form FIRST before watching
const form = ref({
  id: null,
  title: '',
  slug: '',
...
// THEN set up the watch after form is defined
watch(() => form.value.title, (newTitle, oldTitle) => {
  if (newTitle && (!form.value.slug || form.value.slug === generateSlug(oldTitle))) {
...
// Format date for input (convert from API format to YYYY-MM-DD)
const formatDateForInput = (dateString) => {
  if (!dateString) return ''
  return dateString.split('T')[0] // Converts "2026-06-29T15:00:00.000000Z" to "2026-06-29"
}
...
const fetchPublication = async () => {
  ...
  try {
    ...
    // The response structure: { data: { ... }, status: true, alert: {...} }
    ...
    form.value = {
      ...
      date: formatDateForInput(publication.date), // Format date for date picker
      doi: publication.doi || '',
      url: publication.url || '',
      ...
      authors: publication.authors?.map((author, index) => ({
        id: author.id, // Keep author ID for reference
    ...
    console.error(error)
    ElMessage.error('Failed to fetch publication data')
  } finally {
    loading.value = false
  }
}
...
const submit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      // Prepare payload - remove id fields that shouldn't be sent
      const payload = {
        ...
        authors: form.value.authors.map(author => ({
          member_id: author.member_id || null,
          author_name: author.author_name,
      ...
      const errorMsg = error.response?._data?.message || error.message || 'Failed to update publication'
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="Featured Publication">
                ...
                <span class="ml-2 text-gray-500 text-sm">Show as featured</span>
              </el-form-item>
            </el-col>
          </el-row>
