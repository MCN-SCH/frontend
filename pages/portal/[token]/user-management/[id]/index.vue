...
const form = ref({
  ...
  // Member fields (directly on the form, not nested)
  name: '',
  username: '',
  position: '',
  affiliation: '',
...
const fetchMember = async () => {
  ...
  try {
    ...
    // The response data is the member object directly
    form.value = {
      ...form.value,
      // User fields from the response
      email: response.email || '',
      phone: response.phone || '',
      role: String(response.role || ''), // Cast to string to match el-option values
      status: String(response.user?.status || '1'),

      // Member fields from the response (directly on the object)
      name: response.name || '',
      username: response.username || '',
      position: response.position || '',
      affiliation: response.affiliation || '',
      // research_domains is already an array from the API
      research_domains: Array.isArray(response.research_domains)
        ? response.research_domains
        : (response.research_domains ? JSON.parse(response.research_domains) : []),
      website: response.website || '',
      // academic_profiles is already an array from the API
      academic_profiles: Array.isArray(response.academic_profiles)
        ? response.academic_profiles
        : (response.academic_profiles ? JSON.parse(response.academic_profiles) : []),
...
const submit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      // Member fields (directly, not nested)
      formData.append('name', form.value.name)
      formData.append('username', form.value.username)
      formData.append('position', form.value.position || '')
      formData.append('affiliation', form.value.affiliation || '')
      formData.append('website', form.value.website || '')
      formData.append('join_date', form.value.join_date || '')
      formData.append('is_active', form.value.is_active ? '1' : '0')
      formData.append('order', String(form.value.order || 0))

      // Send research_domains as JSON string
      formData.append(
        'research_domains',
        JSON.stringify(form.value.research_domains)
      )

      // Send academic_profiles as JSON string
      formData.append(
        'academic_profiles',
        JSON.stringify(form.value.academic_profiles)
      )
      ...
      <el-card v-loading="loading" shadow="never">
        ...
        >
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="24">
              <el-form-item label="Research Domains" prop="research_domains">
                <el-select
                  v-model="form.research_domains"
                  multiple
                  filterable
                  allow-create
            ...
            <el-col :span="24">
              <el-form-item label="Academic Profiles" prop="academic_profiles">
                <el-select
                  v-model="form.academic_profiles"
                  multiple
                  filterable
                  allow-create
                  default-first-option
